...
app.config['MAIL_PASSWORD'] ='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'
...
def unique_code_generator():
    ...
    chars=string.ascii_lowercase
    string1=''.join(random.choice(chars) for x in range(size))
    string2=str(random.randint(10000,99999))
    code=string1+string2
...
def otp_message():
    ...
    if(request.method=='POST'):
        ...
        if(session['otp']==otp_value):
            ...
            mycursor.execute(query)
            query_create="create table {}(name text,email text UNIQUE,password text,mobile_no integer,roll_no text,class_code text)".format(session['class_code'])
            ...
            create_submit_assignment="create table submitassignment_{}(name text,subject text,roll_no text,file_name text,date blob,status)".format(session['class_code'])
            mycursor.execute(create_submit_assignment)
            ...
            create_images_table="create table images_{}(image_name text)".format(session['class_code'])
            mycursor.execute(create_images_table)
            print(session['class_code'])
            create_chat_table="create table chat_{}(name text,date blob,time blob,chat_text text)".format(session['class_code'])
            mycursor.execute(create_chat_table)
...
@app.route('/main')
def main():
  return render_template('main.html',name=session['Sname'],notification=session['notification'],code=session['class_code'])
...
def attendance():
    ...
    try:
        ...
        return render_template('attendance.html',attendance_status=attendance_status,name=session['Sname'],percent=present_percent,code=session['class_code'])
    except:
...
def take_attendance():
    ...
    return render_template('take_attendance.html',name_list=students_name,roll_no_list=roll_no,name=session['tname'],code=session['class_code'])
...
def forget_message():
    if(request.method=='POST'):
      ...
      if account_type=="teacher":
          forgetquery="select password from teachers_list where email='{}'".format(forget_email)
          mycursor.execute(forgetquery)
          user=mycursor.fetchall()
          session['forget_pass']=user
      if account_type=="student":
          forgetquery="select password from {} where email='{}'".format(class_code,forget_email)
          mycursor.execute(forgetquery)
          user=mycursor.fetchall()
          session['forget_pass']=user
      ...
      print(len(user))
      if len(user)>0:
        session['otp']=send_otp(forget_email)
...
def submit_otp_forget():
    if(request.method=='POST'):
        ...
        if(session['otp']==forget_otp):
            return render_template('submit_otp_forget.html',password=session['forget_pass'])
        else:
            return render_template('submit_otp_forget.html',password="wrong otp")
@app.route('/Edit_notification')
def Edit_notification():
  return render_template("Edit_notification.html",code=session['class_code'],name=session['tname'])
...
def get_notificaton_form_data():
    ...
    return render_template("get_notification_form_data.html",message="successfully ADD",name=session['tname'],code=session['class_code'])
...
def give_marks():
    ...
    roll_no=[row[0] for row in mycursor.fetchall()]
    print(roll_no)
    return render_template('give_marks.html',roll_no=roll_no,code=session['class_code'],name=session['tname'])
...
def Data_give_marks():
    if request.method=='POST':
        ...
        mydb.commit()
        return render_template("Data_give_marks.html",message="success fully updated",name=session['tname'],code=session['class_code'])
@app.route('/my_marks')
def my_marks():
    roll_no_query = """select roll_no from {} where email="{}" """.format(session['class_code'],session['lemail'])
    mycursor.execute(roll_no_query)
    ...
    return render_template("my_marks.html",exam_type=result,name=session['Sname'],code=session['class_code'])
...
def result_data():
    if request.method=="POST":
       ...
       return render_template("my_marks.html", result=result, exam_name=Ename,exam_type=Eresult,code=session['class_code'],name=session['Sname'])
@app.route('/give_assignment')
def give_assignment():
    return render_template("give_assignment.html" ,name=session['tname'],code=session['class_code'])
...
def data_give_assignment():
    if request.method=="POST":
        ...
        unique_filename=unique_code_generator()+assignment_filename
        cloud_path='{}_uploads/{}'.format(session['class_code'],unique_filename)
        storage.child(cloud_path).put(assignment_file)
        print(assignment_filename)
        assignment_query="""insert into assignment_{} values(current_date,"{}","{}","{}","{}")""".format(session['class_code'],subject,assignment,last_date,unique_filename)
...
@app.route('/submit_assignments_data',methods=['POST'])
def submit_assignments_data():
    if request.method=="POST":
        file=request.files['file']
        subject=request.form.get('sub')
        roll_no=request.form.get('roll_no')
        assignment_filename=file.filename.replace(" ","")
        unique_filename=unique_code_generator()+assignment_filename
        cloud_path='{}_uploads/{}'.format(session['class_code'],unique_filename)
        storage.child(cloud_path).put(file)
        submit_assignment_query="""insert into submitassignment_{} values("{}","{}","{}","{}","not verified",current_date)""".format(session['class_code'],session['Sname'],subject,roll_no,unique_filename)
        mycursor.execute(submit_assignment_query)
        mydb.commit()
        return render_template('submit_assignments.html',name=session['Sname'],code=session['class_code'],message="successfully submitted")

@app.route('/check_assignment')
def check_assignment():
    assignment_query="select * from submitassignment_{}".format(session['class_code'])
    mycursor.execute(assignment_query)
    assignment_detail=mycursor.fetchall()
    return render_template('check_assignment.html',assignment_detail=assignment_detail,name=session['tname'],code=session['class_code'])
"""@app.route('/verify_assignment_page')
def verify_assignment_page():
    return render_template('verify_assignment_page.html',name=session['tname'],code=session['class_code'])
@app.route('verify_assignment') """   
...
def assignment():
    ...
    result=mycursor.fetchall()
    print(result)
    return render_template("assignments.html",result=result,name=session['Sname'],code=session['class_code'])
...
def set_fee():
    ...
    mycursor.execute(roll_no_query)
    roll_no =[row[0] for row in mycursor.fetchall()]
    return render_template('set_fee_status.html',name=session['tname'],code=session['class_code'],roll_no=roll_no)
...
def set_fee_data():
    if request.method=="POST":
        ...
        roll_no =[row[0] for row in mycursor.fetchall()]
        mydb.commit()
        return render_template('set_fee_status.html',message="successfully updated",name=session['tname'],code=session['class_code'],roll_no=roll_no)
...
def fee_status():
    ...
    return render_template('fee_status.html',result=result,name=session['Sname'],code=session['class_code'])
@app.route('/upload_images')
def upload_images():
  return render_template("upload_images.html",name=session['tname'],code=session['class_code'])
...
def image_data():
    if request.method=="POST":
        ...
        print(files)
        for file in files:
            unique_filename=unique_code_generator()+file.filename
            print(unique_filename)
            cloud_path='{}_E_images/{}'.format(session['class_code'],unique_filename)
            storage.child(cloud_path).put(file)
            query_file="""insert into images_{} values("{}")""".format(session['class_code'],unique_filename)
            mycursor.execute(query_file)
            mydb.commit()
...
@app.route('/images')    
def images():
    image_query="select image_name from images_{}".format(session['class_code'])
    mycursor.execute(image_query)
    image_result=mycursor.fetchall()
    print(image_result)
    links=[]
    for image in image_result:
        download_path ='{}_uploads/{}'.format(session['class_code'],image)
        link=storage.child(download_path).get_url(None)
        links.append(link)
        print(links)
    
    
    return render_template('images.html',all_links=links,code=session['class_code'],name=session['Sname'])
@app.route('/chat')
def chat():
    query_students="select name from {}".format(session['class_code'])
    mycursor.execute(query_students)
    names=mycursor.fetchall()
    query_chats="select * from chat_{}".format(session['class_code'])
    mycursor.execute(query_chats)
    chat_data=mycursor.fetchall()
    print(chat_data)
    myname=session['Sname']
    return render_template('chat.html',classmates=names,chat_data=chat_data,myname=myname,code=session['class_code'],name=session['Sname'])
